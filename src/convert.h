...
struct result_data_entry
{
    ...
    template <typename Str, typename T>
        requires(std::constructible_from<variant_t, T &&> &&
                 std::constructible_from<QString, Str &&>)
    [[nodiscard]] explicit(!std::convertible_to<T&&, variant_t>)
        result_data_entry(Str&& str_source,
                          T&& dat) noexcept(std::is_nothrow_constructible_v<QString, Str&&> &&
    ...
    void set_error(const QString& err) { data.emplace<std::string>(err.toStdString()); }
...
[[nodiscard]] inline QImage byte_to_QRCode_qimage(const std::string& text,
                                                  const QRcode_create_config qrcode_config)
{
    ZXing::MultiFormatWriter writer(qrcode_config.format);
    writer.setMargin(qrcode_config.margin);

    const auto bitMatrix =
        writer.encode(text, qrcode_config.target_width, qrcode_config.target_height);
...
struct result_i2t
{ //image to text result, 傻瓜式expected
    enum errcode
    {
        success,
        empty_img,
...
[[nodiscard]] inline result_i2t QRcode_to_byte(const std::string& bytes)
{
    ...
    const ZXing::ImageView imageView(
        grayImg.data, grayImg.cols, grayImg.rows, ZXing::ImageFormat::Lum);
